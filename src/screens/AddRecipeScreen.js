...
import { View, Text, StyleSheet, Button, ScrollView, Image, TouchableOpacity, SafeAreaView, TextInput, Alert, KeyboardAvoidingView, Platform } from 'react-native';
...
// The schema no longer validates imageUrl, as it's now a file.
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }} keyboardVerticalOffset={100}>
        <Formik
          initialValues={{ title: '', cookTime: '', ingredients: [''], instructions: [''] }}
          ...
          {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
            <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
              ...
              <View style={styles.imagePickerContainer}>
                <Button title="Pick an image for the recipe" onPress={pickImage} />
                {image && <Image source={{ uri: image.uri }} style={styles.imagePreview} />}
              </View>

             
              <CustomInput label="Recipe Title" onChangeText={handleChange('title')} onBlur={handleBlur('title')} value={values.title} error={errors.title} touched={touched.title} />
              <CustomInput label="Cook Time (e.g., 30 minutes)" onChangeText={handleChange('cookTime')} onBlur={handleBlur('cookTime')} value={values.cookTime} error={errors.cookTime} touched={touched.cookTime} />
                  ...
                  <View style={styles.arrayContainer}>
                    ...
                    {values.ingredients.map((_, index) => (
                      <View key={index} style={styles.arrayItem}>
                        <TextInput style={styles.arrayInput} placeholder={`Ingredient #${index + 1}`} onChangeText={handleChange(`ingredients[${index}]`)} onBlur={handleBlur(`ingredients[${index}]`)} value={values.ingredients[index]} />
                ...
                {({ push, remove }) => (
                  <View style={styles.arrayContainer}>
                    ...
                    {values.instructions.map((_, index) => (
                      <View key={index} style={styles.arrayItem}>
                        <TextInput style={styles.arrayInput} placeholder={`Step #${index + 1}`} onChangeText={handleChange(`instructions[${index}]`)} onBlur={handleBlur(`instructions[${index}]`)} value={values.instructions[index]} multiline />
...
const styles = StyleSheet.create({
  ...
  imagePickerContainer: { alignItems: 'center', marginBottom: 20, padding: 10, borderWidth: 1, borderColor: '#ddd', borderRadius: 8 },
  ...
  arrayInput: { flex: 1, borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 10, marginRight: 10, fontSize: 16 },
