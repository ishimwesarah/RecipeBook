...
const CreateNewsletterScreen = () => {
  ...
  const [document, setDocument] = useState([{ id: generateId(), type: 'paragraph', text: '' }]);

  const pickHeroImage = async () => {
    const permissionResult = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (permissionResult.granted === false) {
      Alert.alert("Permission Required", "You must grant permission to access your photo library.");
      return;
    }
  ...
  const addImageBlock = async () => {
    ...
    try {
      ...
      setDocument(current => current.map(b => b.id === tempId ? newImageBlock : b));
    } catch (error) {
  ...
  const handlePublish = async () => {
    if (!title.trim() || !heroImage) {
      return Alert.alert("Missing Fields", "Please provide a title and a hero image.");
    }
    if (document.length === 1 && !document[0].text.trim()) {
      return Alert.alert("Missing Content", "Article content cannot be empty.");
    }

    setIsSubmitting(true);
    const formData = new FormData();
    formData.append('image', { uri: heroImage.uri, name: `hero_${Date.now()}.jpg`, type: 'image/jpeg' });
    ...
    try {
      ...
      const message = error.response?.data?.message || 'Could not publish the newsletter. Please check your connection.';
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={handlePublish} disabled={isSubmitting} style={styles.publishButton}>
            {isSubmitting ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.publishButtonText}>Publish</Text>}
          </TouchableOpacity>
        </View>

        <ScrollView style={styles.formContainer} keyboardShouldPersistTaps="handled">
          <TouchableOpacity style={styles.heroImagePicker} onPress={pickHeroImage}>
            {heroImage ? <Image source={{ uri: heroImage.uri }} style={styles.heroImagePreview} /> : <View style={styles.heroImagePlaceholder}><Ionicons name="camera" size={40} color="#ccc" /><Text style={styles.heroImagePlaceholderText}>Tap to select a hero image</Text></View>}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  publishButton: { backgroundColor: '#4CAF50', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20, minWidth: 80, alignItems: 'center' },
  publishButtonText: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
  formContainer: { padding: 15 },
  heroImagePicker: {
    height: 200, backgroundColor: "#f5f5f5", borderRadius: 8, justifyContent: "center",
    alignItems: "center", marginBottom: 20, borderWidth: 2, borderColor: "#ddd", borderStyle: "dashed",
  },
  ...
  documentContainer: { borderWidth: 1, borderColor: "#ddd", borderRadius: 8, padding: 15, minHeight: 400 },
  ...
  paragraphInput: { fontSize: 16, lineHeight: 24, paddingVertical: 5, marginBottom: 15 },
